  ...
  programs.nixvim = {
    keymaps = [
      ...
      }
      {
        mode = "n";
        key = "<C-Left>";
        action = ":vertical resize +3<CR>";
        options.desc = "Increase vertical split size";
      }
      {
        mode = "n";
        key = "<C-Right>";
        action = ":vertical resize -3<CR>";
        options.desc = "Decrease vertical split size";
      }
      {
        mode = "n";
        key = "<C-Up>";
        action = ":horizontal resize +3<CR>";
        options.desc = "Increase horizontal split size";
      }
      {
        mode = "n";
        key = "<C-Down>";
        action = ":horizontal resize -3<CR>";
        options.desc = "Decrease horizontal split size";
      }
      {
        mode = "n";
        key = "<C-n>";
      ...
      }
      {
        ...
        options.desc = "Replace all instances of highlighted words";
      }
      {
        mode = "v";
        key = "<C-s>";
        action = ":sort<CR>";
        options.desc = "Sort highlighted text";
      }
      {
        mode = "v";
        key = "J";
      ...
      }
      {
        ...
        key = "G";
        action = "Gzz";
        options.desc = "Move to bottom and center";
      }
      {
        mode = "n";
        key = "<leader>u";
        action = "<Esc><Cmd>UndotreeToggle<CR>";
      }
      {
        mode = "v";
        key = "<leader>ir";
      ...
      }
      {
        ...
        key = "<leader>mk";
        action =
          "<Esc><Cmd>lua function() conform.format({lsp_fallback = true, async = false, timeout_ms = 500,})";
        options.silent = true;
      }
    ];
